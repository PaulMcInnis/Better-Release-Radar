...
TODAY_PICKLE_FILE = os.path.join(
    CACHE_DIR,
    f".followed_artists_{TODAY_DATE}.pkl",
)
...
def delete_old_cache_files(directory, max_age_days=30):
    """Delete files in the given directory that are older than the max age."""
    now = time.time()
...
if __name__ == "__main__":
    ...
    delete_old_cache_files(CACHE_DIR, max_age_days=60)
    ...
    for art in artists:
        for alb in art["albums"]:
            ...
            if (
                args.region not in alb["available_markets"]
